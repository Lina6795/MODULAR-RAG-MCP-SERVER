...
class TestGeminiEmbedding:
    """Test suite for GeminiEmbedding implementation."""

    def test_initialization_uses_gemini_base_url(self, mock_settings_openai: Any) -> None:
        """Gemini should normalize to the native Google embeddings base URL."""
        mock_settings_openai.embedding.provider = "gemini"
        mock_settings_openai.embedding.model = "gemini-embedding-001"
        mock_settings_openai.embedding.dimensions = 768
        mock_settings_openai.embedding.base_url = (
            "https://generativelanguage.googleapis.com/v1beta/openai/"
        )

        embedding = GeminiEmbedding(mock_settings_openai, api_key="test-key")

        assert embedding.api_key == "test-key"
        assert embedding.base_url == GeminiEmbedding.DEFAULT_BASE_URL
        assert embedding.get_dimension() == 768

    @patch("httpx.Client")
    def test_embed_success(self, mock_httpx_client: Mock, mock_settings_openai: Any) -> None:
        """Gemini embedding should call the native batch endpoint."""
        mock_settings_openai.embedding.provider = "gemini"
        mock_settings_openai.embedding.model = "gemini-embedding-001"
        mock_settings_openai.embedding.dimensions = 768
        ...
        assert call_args.args[0].endswith("/models/gemini-embedding-001:batchEmbedContents")
        assert call_args.kwargs["json"]["requests"][0]["outputDimensionality"] == 768

    @patch("httpx.Client")
    def test_embed_http_error(self, mock_httpx_client: Mock, mock_settings_openai: Any) -> None:
        """Gemini embedding should raise a provider-specific error on HTTP failures."""
        mock_settings_openai.embedding.provider = "gemini"
        mock_settings_openai.embedding.model = "gemini-embedding-001"
        mock_settings_openai.embedding.dimensions = 768
        ...
        request = Mock()
        error = httpx.HTTPStatusError("boom", request=request, response=mock_response)
        ...
        embedding = GeminiEmbedding(mock_settings_openai, api_key="test-key")
        with pytest.raises(GeminiEmbeddingError, match="HTTP 404"):
...
class TestEmbeddingFactoryRegistration:
    ...
    def test_gemini_registered(self) -> None:
        """Test that Gemini provider is registered with factory."""
